...
if (isset($nom)) {
    ...
    // Depot des images dans le dossier photo
    ...
    if(!empty($nom) && !empty($email) && !empty($date) && !empty($phone) && !empty($sexe) && !empty($photo))
    {
                     if(filter_var($_POST['email'],FILTER_VALIDATE_EMAIL))
                {
                    ...
                    $user=$req->fetch();
                    if($user){
                    $erreur='Cet email est deja utilisé dans un autre Etudiant';

                            }else{
                                        /*  incscription de l'utilisateur  */  
                                $q= $connexion->prepare("INSERT INTO etudiant SET nom=?,email=?,daten=?,telephone=?,sexe=?,photo=?,datime=now()");
	...
	$details="Inscription d'un étudiant";
	$contenu="contenu/etudiant.php";
